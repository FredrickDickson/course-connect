...
SET 
  ...
  pathway_tags = CASE 
    WHEN title ILIKE '%mediation%' OR tags && ARRAY['mediation'] THEN 
      array_distinct(COALESCE(tags, ARRAY[]::TEXT[]) || ARRAY['mediation'])
    WHEN title ILIKE '%arbitration%' OR tags && ARRAY['arbitration'] THEN 
      array_distinct(COALESCE(tags, ARRAY[]::TEXT[]) || ARRAY['arbitration'])
...
CREATE INDEX IF NOT EXISTS idx_course_enrollments_pathway_status ON course_enrollways(pathway, status);
...
CREATE OR REPLACE VIEW user_pathway_summary AS
SELECT 
  u.id as user_id,
  u.first_name,
  u.last_name,
