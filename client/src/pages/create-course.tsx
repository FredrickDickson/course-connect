...
export default function CreateCourse() {
  const { isLoading: authLoading, hasAccess } = useRoleProtection({ requiredRole: 'instructor' });
  const { t } = useLanguage();
  ...
  const onSubmit = (data: CourseFormData) => {
    if (data.categoryId === CUSTOM_CATEGORY_VALUE && !customCategory.trim()) {
      toast({ title: "Error", description: "Please enter a custom category name", variant: "destructive" });
      return;
    }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center gap-4 mb-8">
          <Button variant="ghost" size="sm" asChild>
            <Link href="/instructor"><ArrowLeft className="h-4 w-4 mr-2" />Back to Dashboard</Link>
          </Button>
          <div>
            <h1 className="text-3xl font-bold text-foreground">Create New Course</h1>
            <p className="text-muted-foreground mt-2">Fill in the details to create your new course</p>
          </div>
        </div>

        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><BookOpen className="h-5 w-5" />Course Information</CardTitle>
          </CardHeader>
          <CardContent>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  <div className="space-y-6">
                    ...
                    <FormField control={form.control} name="subtitle" render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl><Input placeholder="Enter course subtitle" {...field} /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                      ...
                      <FormItem>
                        ...
                        <FormControl>
                          <div className="space-y-4">
                            <ImageUploader currentImageUrl={field.value} onUploadComplete={(url) => field.onChange(url)} />
                            ...
                            <Input type="url" placeholder="https://example.com/image.jpg" value={field.value || ''} onChange={field.onChange} />
                          </div>
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                      ...
                      <FormField control={form.control} name="isPublished" render={({ field }) => (
                        <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                          <div className="space-y-0.5">
                            ...
                            <div className="text-sm text-muted-foreground">Make this course available to students</div>
                          </div>
                      ...
                      <FormField control={form.control} name="isFeatured" render={({ field }) => (
                        <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                          <div className="space-y-0.5">
                            ...
                            <div className="text-sm text-muted-foreground">Display this course prominently on the homepage</div>
                          </div>
                          <FormControl><Switch checked={field.value} onCheckedChange={field.onChange} /></FormControl>
                        </FormItem>
                      )} />
                ...
                <FormField control={form.control} name="description" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea placeholder="Describe what students will learn in this course..." rows={6} {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                ...
                <div className="flex items-center justify-end gap-4 pt-4 border-t">
                  <Button type="button" variant="outline" asChild><Link href="/instructor">Cancel</Link></Button>
                  <Button type="submit" disabled={createCourse.isPending}>
                    {createCourse.isPending ? (
                      <><div className="animate-spin w-4 h-4 border-2 border-primary border-t-transparent rounded-full mr-2" />Creating...</>
                    ) : (
